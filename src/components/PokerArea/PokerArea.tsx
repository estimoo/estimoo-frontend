...
const PokerArea: React.FC<PokerAreaProps> = ({ onVote, currentVote }) => {
  const pokerCards = [
    { id: 1, value: '0' },
    { id: 2, value: '1' },
    { id: 3, value: '2' },
    ...
    <div className="flex-1 rounded-lg p-3">
      <div className="flex justify-center items-center gap-4 flex-wrap h-full">
        {pokerCards.map((card) => (
          <div
            key={card.id}
            className={`relative rounded-lg border-4 text-white shadow-md hover:scale-105 transition-transform duration-200 cursor-pointer
              ${currentVote === card.value
                ? 'border-blue-500 bg-blue-900'
                : 'border-neutral-700 bg-base-200'
              }`}
